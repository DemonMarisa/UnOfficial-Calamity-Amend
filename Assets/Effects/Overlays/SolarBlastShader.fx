...
float4 SolarBlastFunction(float2 coords : TEXCOORD0) : COLOR0
{
    ...
    // 最终进行染色
    float4 OutPutColor = lerp(ubeginColor, uendColor, originalColor.r);
    
    if (UseColor)
        return float4(OutPutColor.rgb, originalColor.a);
    else
        return originalColor * uOpacity;
}

// 最终通道
technique Technique1
{
    pass UCASolarBlastPass
    {
        PixelShader = compile ps_2_0 SolarBlastFunction();
    }
}