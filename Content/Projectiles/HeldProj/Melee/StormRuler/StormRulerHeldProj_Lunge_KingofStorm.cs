...
{
    // 直接复制的，懒了
    public class StormRulerHeldProj_Lunge_KingofStorm : ModProjectile, ILocalizedModType, IPixelatedRenderer
    {
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            {
                float _ = float.NaN;
                Vector2 beamBeginPos = BeginPos - Projectile.rotation.ToRotationVector2() * 600;
                Vector2 beamEndPos = BeginPos + Projectile.rotation.ToRotationVector2() * 1200;
        ...
        public void Init()
        {
            ...
            if (Projectile.IsLocalPlayer())
            {
                ...
                if (HasFocus)
                    damage = (int)(Projectile.damage * UCABlanceRule.StormRulerKingofStormDamageMult);
                else
                    damage = (int)(Projectile.damage * UCABlanceRule.StormRulerKingofStormNoFocusDamageMult);
        ...
        public void RenderPixelated(SpriteBatch spriteBatch)
        {
            ...
            Vector2 drawPosition = BeginPos - Main.screenPosition + Vector2.UnitX.RotatedBy(Projectile.rotation + ProjRotOffset) * 1200;
            Vector2 orig = new Vector2(texture.Width, texture.Height / 2);
            Main.spriteBatch.Draw(texture, drawPosition, null, Color.White * 0.755f * Projectile.Opacity, Projectile.rotation + ProjRotOffset, orig, new Vector2(9f, 0.6f), SpriteEffects.FlipHorizontally, 0f);

            Main.spriteBatch.Draw(texture, drawPosition, null, Color.White * 0.4f * Projectile.Opacity, Projectile.rotation + ProjRotOffset, orig, new Vector2(11f, 0.2f), SpriteEffects.FlipHorizontally, 0f);
