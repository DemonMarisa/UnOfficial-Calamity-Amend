...
{
    public class StormShockWaveSmall : BaseMeleeProj, IPixelatedRenderer
    {
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            if (targetHitbox.Intersects(projHitbox))
            {
                ...
                {
                    Rectangle ProjHitbox = Utils.CenteredRectangle(CenterPos[i] + Projectile.Center - Projectile.velocity, new Vector2(50, 50));
                    if (targetHitbox.Intersects(ProjHitbox))
                        return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                int Index = Main.rand.Next(10, CenterPos.Count - 10);
                Vector2 RandomPos = CenterPos[Index] + Projectile.Center;
                new Fire(RandomPos, -Projectile.velocity * Main.rand.NextFloat(0f, 0.4f), Color.White, 25, Main.rand.NextFloat(MathHelper.TwoPi), 0.5f, Main.rand.NextFloat(0.3f, 1f)).Spawn();
            }
            for (int i = 0; i < 2; i++)
            {
            ...
            Projectile.Opacity = MathHelper.Lerp(Projectile.Opacity, 1f, 0.4f);
        }
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Projectile.LAP().OnceHitEffect)
            {
                Vector2 Center = Projectile.Center + new Vector2(540, 0).RotatedBy(Projectile.rotation);
                Projectile p = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Center, Vector2.Zero, ProjectileType<StormBlast>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
        ...
        public void RenderPixelated(SpriteBatch spriteBatch)
        {
            ...
            DrawBaseWave(texture, Color.Gray * 0.8f, 2f);
            DrawBaseWave(texture, Color.White * 0.5f, 6f);
            DrawBaseWave(texture, Color.White * 0.5f, 20f);
            ...
            if (!LAPConfig.Instance.PerformanceMode)
            {
                ...
                LAPUtilities.ApplyAlphaCut(vector4, new(0, -Main.GlobalTimeWrappedHourly * 0.35f + RanddomOffset4), new Vector2(3f, 0.1f), Color.White);
                DrawBaseWave(texture2, Color.White, 30f);
            }
